    ...
    private void MainWindowUI(int windowId)
    {
        ...
        { // Main Window Area
            ...
            GUILayout.BeginVertical();
            {
                ...
                // Interactive mode cheat
                bool interactiveMode = GameManager.Instance.interactiveMode;
                interactiveMode = GUILayout.Toggle(interactiveMode, "Interactive mode");
                if (GUI.changed)
                ...
                volume = (int) GUILayout.HorizontalSlider(volume, -80f, 20f, GUILayout.ExpandWidth(true));
                if (GUI.changed)
                    SoundManager.Instance.masterVolume = volume;
                
                // Audio mute
                bool muted = SoundManager.Instance.masterMuted;
                muted = GUILayout.Toggle(muted, "Audio muted");
                if (GUI.changed)
